...
abstract class SimpleFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        tf = Typeface.createFromAsset(requireActivity().assets, "OpenSans-Regular.ttf")
    ...
    protected fun generateLineData(): LineData {
        ...
        val ds1 = LineDataSet(
            FileUtils.loadEntriesFromAssets(
                requireActivity().assets, "sine.txt"
            ), "Sine function"
        )
        val ds2 = LineDataSet(
            FileUtils.loadEntriesFromAssets(
                requireActivity().assets, "cosine.txt"
            ), "Cosine function"
        )
        ds1.lineWidth = 2f
        ds2.lineWidth = 2f
    ...
    protected val complexity: LineData
        protected get() {
            ...
            val ds1 = LineDataSet(
                FileUtils.loadEntriesFromAssets(
                    requireActivity().assets, "n.txt"
                ), "O(n)"
            )
            val ds2 = LineDataSet(
                FileUtils.loadEntriesFromAssets(
                    requireActivity().assets, "nlogn.txt"
                ), "O(nlogn)"
            )
            val ds3 = LineDataSet(
                FileUtils.loadEntriesFromAssets(
                    requireActivity().assets, "square.txt"
                ), "O(n\u00B2)"
            )
            val ds4 = LineDataSet(
                FileUtils.loadEntriesFromAssets(
                    requireActivity().assets, "three.txt"
                ), "O(n\u00B3)"
            )
            ds1.color = ColorTemplate.VORDIPLOM_COLORS[0]
            ds2.color = ColorTemplate.VORDIPLOM_COLORS[1]
